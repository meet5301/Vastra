...
async function loadMenProducts() {
  ...
  const params = new URLSearchParams({ page: currentPage, limit: 16, category: "Men", ...(currentSort && { sort: currentSort }), ...(typeVal && { subCategory: typeVal }) });
  try {
    ...
    if (!data.success || !products.length) { grid.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:60px;color:#888;">No products found.</div>`; return; }
...
function goPage(p) { currentPage = p; loadMenProducts(); document.getElementById('products')?.scrollIntoView({ behavior: 'smooth' }); }
