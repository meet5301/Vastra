// ── VASTRA MEN PAGE JS ────────────────────────────────────────────
const MEN_FALLBACK_IMAGE = "https://images.unsplash.com/photo-1617137968427-85924c800a22?w=400&h=500&fit=crop";
  ...
  return `
    <div class="men-prod-card" onclick="window.location.href='/detail?id=${p._id}'">
      <div class="men-prod-img-wrap">
        <img src="${p.image}" loading="lazy" onerror="this.onerror=null;this.src='${fallback || MEN_FALLBACK_IMAGE}'" />
...
const menPageController = window.createAdvancedCategoryPage({
  category: "Men",
  fallbackImage: MEN_FALLBACK_IMAGE,
  renderCard,
});

function loadMenProducts() {
  menPageController.loadProducts();
}
  ...
  try {
    ...
    if (data.success) {
      el.innerHTML = (data.products || [])
        .map((p) => renderCard({ ...p, image: p.image || MEN_FALLBACK_IMAGE }, MEN_FALLBACK_IMAGE))
        .join("");
    }
  } catch {}
}
