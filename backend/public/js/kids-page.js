...
async function loadKidsProducts() {
  ...
  const params = new URLSearchParams({ page: currentPage, limit: 16, ...(currentSort && { sort: currentSort }) });
  try {
...
function goPage(p) { currentPage=p; loadKidsProducts(); document.getElementById('products')?.scrollIntoView({behavior:'smooth'}); }
